...
Using  [Theorem 3](#FGA-2-theorem-3), we now obtain \emph{existence} theorems for sheaves, for morphisms of schemes, or for schemes.
...
It follows easily that we have an \emph{obstruction of constructing $X_n$ lifting $X_{n-1}$}, which lives in $H^2(X_0,\frak{G}_{X_0/k})\otimes\frak{m}^n/\frak{m}^{n+1}$.
Then:
...
\oldpage{182-14}
  of formal schemes $\frak{X}$ over $A$, of finite type, and flat over $A$, such that $X\otimes_Ak=X_0$.
  With this notation:
  \begin{enumerate}[i.]
    \item if $H^1(X_0,\frak{G}_{X_0/k})=0$, then, for all $A$, $F(A)$ has at most one element;
    \item if $H^2(X_0,\frak{G}_{X_0/k})=0$, then, for all $A$, $F(A)$ has at least one element.
  \end{enumerate}
\end{itenv}
...
We can use \hyperref[theorem3corollary2]{Corollary 2} of  [Theorem 3](#FGA-2-theorem-3).
For example:
\begin{itenv}{Corollary 2}
...
\begin{rmenv}{Remarks}
  \begin{enumerate}[1)]
    \item \hyperref[theorem9corollary3]{Corollary 3} and \hyperref[theorem9corollary4]{Corollary 4} are above all interesting if $k$ is of characteristic $p\neq0$, taking $A$ to be a discrete valuation ring \emph{of characteristic $0$}, with residue field $k$;
      ...
      We must start
\oldpage{182-15}
      by seeing if the first obstruction that we have, in $H^2(X_0,\frak{G}_{X_0/k})$, is necessarily zero.
    \item We note that  [Theorem 3](#FGA-2-theorem-3), and the corresponding technique, only works for a \emph{complete} (local, for simplicity) base ring.
    ...
    \item We will compare the results from this section (mainly the above \hyperref[theorem9corollary1]{Corollary 1} and \hyperref[theorem9corollary2]{Corollary 2}), as well as those from the following, with the results of Kodaira--Spencer on the variation of complex structures.
      ...
      This is what we can prove, at least, when $X_0=\frak{P}_k^r$, by using \hyperref[proposition2corollary1]{Corollary 1} of \hyperref[proposition2]{Proposition 2}.
...
\begin{rmenv}{Remark}
  ...
  \emph{We note that J.-P. Serre has constructed a non-singular projective variety, of dimension $3$, over an algebraically closed field $k$, of characteristic $p>0$, which does not come from reduction of a proper scheme over a local integral ring with residue field $k$, and having a field of fractions of characteristic $0$.}\footnote{\textsc{Serre, J.-P.} "Exemples des variétés projectives en caractéristique $p$ non relevables en caractéristique zéro". \emph{Proc. Nat. Acad. Sc. U.S.A.} \textbf{47} (1961), 108--109.}
...
The most interesting case in which we can apply  [Theorem 11](#FGA-2-theorem-11) is when $f$ is a \emph{separable} morphism.
...
\oldpage{182-24}
    ...
    \item In general, we cannot say anything at the moment about the kernel of $\pi_1(\overline{F})\to\pi_1(X)$, although it should involve a $\pi_2(Y)$.
      It seems, however, that we should be able to prove that $\pi_1(\overline{F})\to\pi_1(X)$ is \emph{injective} if $Y$ is the spectrum of a local ring $A$, by appealing to  [Theorem 12](#FGA-2-theorem-12) below (which shows that this is the case if $A$ is \emph{complete}).
  \end{enumerate}
\end{rmenv}
...
\oldpage{182-27}
  surjective homomorphism from \hyperref[theorem12corollary2]{Corollary 2} of  [Theorem 12](#FGA-2-theorem-12) is an \emph{isomorphism}.
\end{itenv}
