...
\oldpage{C-03}The substance of [§1](#fga-2-section-1) to [§5](#fga-2-section-5) is contained in the published part of [@GD1960, III]; that of [§6](#fga-2-section-6) and [§7](#fga-2-section-7) is contained in [@Gro1960b, III].
For the study of the fundamental group, see [@Gro1960b, V, IX, X, and XI], as well as [@Gro1960b, X, XII, and XIII] for the Lefschetz-type theorems and numerous open questions.
...
\oldpage{182-02}We define an _affine scheme_ to be a ringed space that is isomorphic to some $\Spec(A)$, and a _prescheme_ to be a locally-affine ringed space, i.e. such that every point has an open neighbourhood that is an affine scheme for the induced structure.
...
\oldpage{182-03}Inspired by well-known ideas, we can define the notion of a _projective morphism_, and, more generally, of a _proper morphism_.
...
With $X$ being a (Noetherian, as always) scheme, the sheaf $\scr{O}_X$ is a _coherent sheaf of rings_ in the sense of [@Gro1960].
...
\oldpage{182-04}By the definitions, a formal completion $\overline{X}$ as above is a formal scheme, and, conversely, every formal scheme is _locally_ of this type.
...
the following theorem relates it with "algebraic cohomology", and resembles a well-known theorem of Serre [@Ser1956] on the comparison between algebraic cohomology and analytic cohomology.

::: {.itenv #fga-2-theorem-2 title="Theorem 2" latex="{Theorem 2}"}
\oldpage{182-05}_First comparison theorem._ ---
...
\oldpage{182-06}The latter identity shows that the category of coherent sheaves on $X$ can be identified with a _subcategory_ (with morphisms being the induced morphisms) of the category of coherent sheaves on $\overline{X}$.
In fact, we even have:

::: {.itenv #fga-2-theorem-3 title="Theorem 3" latex="{Theorem 3}"}
For a sheaf of modules on $\overline{X}$ to be coherent, it is necessary and sufficient that it be isomorphic to a sheaf of the form $\overline{\scr{F}}$, where $\scr{F}$ is a coherent sheaf on $X$ (determined up to canonical isomorphism, by [Corollary 3 of Theorem 2](#fga-2-theorem-2-corollary-3)).
...
\oldpage{182-07}Then, under these conditions, $\frak{X}$ is algebraisable, and, more precisely, is isomorphic to $\overline{X}$, where $X$ is some projective $A$-scheme.
:::
...
For the proof of [Theorem 1](#fga-2-theorem-1), [Theorem 2](#fga-2-theorem-2), and [Theorem 3](#fga-2-theorem-3), we refer the reader to [@GD1960, I].
...
Then the set $U$ of points of $X$ that are isolated in their fibre is open, and, if $f$ is quasi-projective^[*[Comp.]* This hypothesis can be replaced by the weaker hypothesis "if $f$ is separated", by means of the following result (see [@Gro1960b, VIII, 6.2]): every morphism $f\colon X\to Y$ which is quasi-finite and separated is also projective.], then $U$ is $Y$-isomorphic to an open subset of some scheme $Y'$ that is finite over $Y$.
:::
...
\oldpage{182-09}This also implies that, for every $y\in Y$, if we filter $\scr{F}$ along the fibre $f^{-1}(y)$ by the $\frak{m}_y^n\scr{F}$ (where $\frak{m}_y$ is the maximal ideal of $\scr{O}_y$), then the associated graded algebra is isomorphic to $(\scr{F}/\frak{m}_y\scr{F})\otimes_{k(y)}\gr(\scr{O}_y)$;
in other words, we have that
$$
  \frak{m}_y^n\scr{F}/\frak{m}_y^{n+1}
  = \scr{F}_y\otimes_{k(y)}(\frak{m}_y^n/\frak{m}_y^{n+1})
$$
for every integer $n$, where $\scr{F}_y$ denotes the sheaf $\scr{F}/\frak{m}_y\scr{F}$ induced by $\scr{F}$ on $X_y$ (with $X_y$ denoting the fibre $f^{-1}(y)$ considered as a proper scheme over the residue field $k(y)$ of $y$).
...
\oldpage{182-10}Let $E_0$ be a locally free coherent sheaf on $X_y$ such that $H^1(X_y;\shHom_{\scr{O}_X}(E_0,E_0))=0$.
...
$$
\oldpage{182-11}(which can also be deduced from the exact sequence in cohomology induced by the exact sequence of _sheaves_ of groups
$$
  e \to \scr{O}_X^\times \to \shAut \to \shAut_Y(X) \to e,
$$
where $\scr{O}_X^\times$ is the sheaf of "units" of $\scr{O}_X$, identified with the centre of $\shAut(E)$.)
...
\oldpage{182-12}It is equivalent to give either such an $X$ or, on the topological space $X_0$, a sheaf $\scr{O}_X$ endowed with the following structures:
...
It follows easily that we have an *obstruction of constructing $X_n$ lifting $X_{n-1}$*, which lives in $H^2(X_0,\frak{G}_{X_0/k})\otimes\frak{m}^n/\frak{m}^{n+1}$.
Then:
...
2. \oldpage{182-15}We note that [Theorem 3](#fga-2-theorem-3), and the corresponding technique, only works for a _complete_ (local, for simplicity) base ring.
...
3. We will compare the results from this section (mainly the above [Corollary 1](#fga-2-theorem-9-corollary-1) and [Corollary 2](#fga-2-theorem-9-corollary-2)), as well as those from the following, with the results of Kodaira--Spencer on the variation of complex structures.
    ...
    In any case, if $H^1(X_0,\frak{G}_{X_0/k})=0$, then we can prove that the fibres of $X$ and $X'$ over any point $y$ of $Y=\Spec(A)$ are isomorphic, or at least when we pass to the algebraic closure of the residue field $k(y)$.
...
::: {.rmenv title="Remark" latex="{Remark}"}
*[Comp.]*
\oldpage{182-16}We note that Mumford has recently constructed schemes of modules for the curves of genus $g$ (cf. _Mumford--Tate seminar_, Harvard University, 1961--62).
...
Let $X$ be a scheme.
\oldpage{182-18}Then an $X$-scheme $X'$ is said to be an _unramified covering of $X$_ if
...
iii. for all $x\in X$, the quotient $\scr{A}_x/\frak{m}_x\scr{A}_x = \scr{A}_x\otimes_{\scr{O}_X}\pi(x)$ is a separable algebra over $k(x)$.
...
More precisely, we define a _geometric point_ of a scheme $X$ to be a morphism $a$ from the spectrum $\xi$ of an algebraically closed field $\Omega$ to $X$, i.e. the data of an algebraically closed extension of the residue field $k(x)$ of a point $x=|a|$ of $X$ (called the _locality_ of the geometric point $a$).
...
\oldpage{182-19}Of course, $\pi_1(X,a)$ is a _covariant functor in the pointed scheme $(X,a)$_.
...
Let $f\colon X\to Y$ be a proper flat morphism such that, for all $y\in Y$, the algebra $H^0(f^{-1}(y),\scr{O}_{f^{-1}(y)})$ is separable over the residue field $k(y)$ (which is the case, for example, if $f^{-1}(y)$ is a _separable scheme_ over $k(y)$, i.e. reduced and such that the fields corresponding to its irreducible components are separable extensions of $k(y)$).
\oldpage{182-21}Then the covering $Y'$ of $Y$ associated to $f_*(\scr{O}_X)$ is unramified.
:::
...
_The kernel of the homomorphism $\pi_1(X)\to\pi_1(Y)$ (which is surjective, by [Lemma 4](#fga-2-lemma-4)) is the closed invariant subgroup generated by the images in $\pi_1(X)$ of $\pi_1(f^{-1}(y))$, where $f^{-1}(y)$ denotes the scheme $f^{-1}(y)\otimes_{k(y)}\overline{k(y)}$ (where $\overline{k(y)}$ denotes an algebraic closure of $k(y)$)._
...
\oldpage{182-23}[Corollary 3](#fga-2-theorem-11-corollary-3) is equivalent to the following statement:
...
2. \oldpage{182-24}In general, we cannot say anything at the moment about the kernel of $\pi_1(\overline{F})\to\pi_1(X)$, although it should involve a $\pi_2(Y)$.
...
\oldpage{182-25}We might hope that this homomorphism is always _bijective_.
...
Suppose now that $X'_1$ is Galois, with Galois group $G$ _of order $n$, coprime to the characteristic $p$ of $k(y_0)$_ (which is also the characteristic of the residue field of $\scr{O}'$).
...
\oldpage{182-28}(Here, [Theorem 9](#fga-2-theorem-9) is actually useless, and it seems that we can deduce the above corollary from the particular case in question in the three point problem).
