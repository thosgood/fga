...
the questions raised in [FGA 3.I, A.2.c](#fga-3-i-section-A.2.c) can probably be answered at the same time as the questions posed in [§2](#fga-3-iii-section-2) of this current talk.
...
We can also express the axioms of a set-theoretical equivalence relation for the $R(T)$ in the $X(T)$ diagrammatically in $\cal{C}$ (under the assumption that both $X\times X$ and the fibre product $(R,p_2)\times_X(R,p_1)$ exist), following the general principle of [FGA 3.II, A.1](#fga-3-ii-section-A.1).
...
Then $f$ is indeed an epimorphism, and even a strict epimorphism (cf.[FGA 3.I, A.2.c](#fga-3-i-section-A.2.c)), with the converse being true if the fibre product $X\times_Y X$ exists.
...
I have only obtained, in this direction, very partial, positive, results, that are vital for the proof of the fundamental theorem of the formal theory of modules (cf. [FGA 3.II, B, Theorem 1](#fga-3-ii-section-B-theorem-1)).
...
This implies (cf. [FGA 3.II, A.1](#fga-3-ii-section-A.1)) that, for every object $T$ of $\cal{C}$, we have a group structure on $G(T)$, and the structure of an operator domain on $X(T)$ acting on $G(T)$, such that, for variable $T$, the structures in question "vary functorially" in $T$.
...
Having recalled these notions, the general definitions in [FGA 3.II, A.1](#fga-3-ii-section-A.1) show, in particular, what we should mean by "the structure of a _$\cal{C}$-category_" (resp. _$\cal{C}$-groupoid_) on a pair of objects $(X,R)$ of an arbitrary category $\cal{C}$:
...
Finally, if $u$ is a monomorphism, then the morphism $G\to G/F$ is flat, and the morphism $G\times F\to G\times_{(G/F)}G$ is an isomorphism, or, in other words, $G$ is a principal homogeneous space over $G/F$, with structure group $F$ (acting on the right), or rather $F\times_S(G/F)$ considered as a group scheme over $G/F$ (cf. [FGA 3.I, B.6](#fga-3-i-section-B.6)).
:::
...
2. Let $\scr{L}'$ be the invertible sheaf on $Y$ induced from $\scr{L}$ by "faithfully flat descent" under $f$ (cf. [FGA 3.I, B, Theorem 1](#fga-3-i-section-B.1-theorem-1)).
