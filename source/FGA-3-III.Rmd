...
*[Comp.]*
We note that the application (of the theory developed here) in talk 3.V ("Picard schemes: existence theorems") can equally be replaced by a suitable use of Hilbert schemes (cf. _Séminaire Mumford--Tate_, Harvard University (1961--62)).
...
An important theorem in this direction has been obtained by Mumford ("An elementary theorem in geometric invariant theory". _Bull. Amer. math. Soc._ **67** (1961), 483--486).
...
\oldpage{212-05}
As we mentioned in [FGA 3.I](FGA-3-I.html), _there are non-effective epimorphisms in $\cal{C}$_ (or even non-strict, which is the same, since fibre products exist).
...
A category should be defined as consisting of two base sets, $X$ and $R$, with the former being the set of _objects_ and the latter the set of _arrows_, endowed with the following structures:
\oldpage{212-08}
...
$$
  p_1,p_2\colon R\rightrightarrows X
$$
called the _source morphism_ and the _target morphism_, and, if the fibre product in question exists, a morphism
\oldpage{212-09}
$$
  \pi\colon (R,p_2)\times_X(R,p_1) \to R
$$
...
Let $S$ be a _locally Noetherian_ prescheme, $X$ a _quasi-projective_ $S$-scheme, and $\cal{R}$ an equivalence pre-relation on $X$, such that:
\oldpage{212-15}
...
\oldpage{212-17}
We can thus apply [Theorem 6.1](#fga-3-iii-theorem-6.1) in order to construct $A/C=B$, which is the desired $B$.
:::
...
Suppose that the group $G=\GP(n)_S$ acts on $X$ and $\scr{L}$ simultaneously (in a way that is compatible with its action on $X$), and that it acts _freely_ on $S$.
\oldpage{212-19}
