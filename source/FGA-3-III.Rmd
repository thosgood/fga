...
\oldpage{212-15}
  ...
  Under these conditions:
  \begin{enumerate}[(i)]
    \item $Y=X/\cal{R}$ exists, and (if $S$ is Noetherian) is quasi-projective\footnote{_[Comp.]_ \emph{The fact that $Y=X/\cal{R}$ is quasi-projective over $S$ has only been proven, for now, in the case where $\cal{R}$ comes from an equivalence relation.}} over $S$.
...
\begin{itenv}{Conjecture 8.1}
  ...
  \begin{enumerate}
    ...
    \item Let $\scr{L}'$ be the invertible sheaf on $Y$ induced from $\scr{L}$ by "faithfully flat descent" under $f$ (cf. [@3-II-1, B, Theorem 1]).
