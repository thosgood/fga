...
Here we will construct "Hilbert schemes", which are meant to replace the use of Chow coordinates, as was mentioned in [FGA 3.II, C.2](FGA-3-II.html#fga-3-ii-section-C.2).
In the next article, the theory of passing to the quotient in schemes, developed in [FGA 3.III](FGA-3-III.html), combined with the theory of Hilbert schemes, will allow us, for example, to construct Picard schemes (defined in [FGA 3.II, C.3](FGA-3-II.html#fga-3-ii-section-C.3)) under rather general conditions.

In summary, we can say that we now have a more or less satisfying technique of projective constructions, apart from the fact that we are still missing^[See the addendum at the end of this article.] a theory of passing to the quotient by groups such as the projective group, acting "without fixed points" (cf. [FGA 3.III, §8](FGA-3-III.html#fga-3-iii-section-8)).
...
\oldpage{221-04}Such flatness properties on suitable sheaves indeed ensure the commutativity of operations such as $\shTor_i(\scr{F},\scr{F}')$ with arbitrary base change.
...
\oldpage{221-06}Recall also that, if $\scr{F}$ is a coherent sheaf on $X$, then it is isomorphic to a quotient sheaf of a sheaf of the form $\OO_X(-n)^N$, for some large enough $n,N$.
...
\oldpage{221-10}As in the previous lemma, we can also reduce to the case where $\scr{F}=\scr{F}_{(r)}$, so that $\scr{F}$ comes from a module on $Y_\scr{F}$.
...
::: {.proof}
\oldpage{221-17}Indeed, if $\scr{G}_s=\scr{F}_s/\scr{H}_s$, it suffices to consider the largest subsheaf $\scr{H}$ of $\scr{F}$ that induces $\scr{H}_s$ ([@GD1960, I, 9.4.2]) and to take $\scr{G}=\scr{F}/\scr{H}$.
...
\oldpage{221-18}we note that, when $X=\PP_S^r$, with $S$ the spectrum of a field, the knowledge of the Hilbert polynomial of a coherent module $\scr{F}$ over $X$ is equivalent to that of the Chern classes of $\scr{F}$, or even of the class of $\scr{F}$ in the ring of classes of coherent sheaves on $X$.
...
c. The preschemes $\repHom_S(X,Y)$, $\prod_{X/S}Z/S$, and $\repIsom_S(X,Y)$, defined in [FGA 3.II, C.2](FGA-3-II.html#fga-3-ii-section-C.2) exist thanks to suitable projective hypotheses, and can be realised as opens in suitable Hilbert preschemes.
...
\oldpage{221-22}Then $x$ is defined by a coherent quotient module $\scr{G}_0=\scr{F}_0/\scr{H}_0$ of the module $\scr{F}_0=F_K$ on the $K$-prescheme $X_K$.
...
\oldpage{221-23}it is then interesting to consider $\scr{A}$ as a module over $Y_0$, and to calculate $\HH^0$ and $\HH^1$ on $Y$.
...
$$
\oldpage{221-26}which allows us to recover the $\scr{N}_{X/S}^u(\scr{F})$.
...
we will not give here the details of the proof, which also uses the result of factorisation of a finite morphism given in [FGA 3.I, A.2.b](FGA-3-I.html#fga-3-i-section-A.2.b).
