...
- (b\textsubscript{s}) The Poincaré polynomials $P_\scr{F}$ of the sheaves $\scr{F}$ whose class is in $E$ have coefficients _in degrees $\leq (s-1)$_ that are bounded.
...
$$
  P_\scr{F} = P_{\scr{F}_{(r)}} + P_{N_r}
$$
then shows that the $P_{N_r}$ satisfy the same condition (b\textsubscript{s}) as the $P_\scr{F}$.
:::