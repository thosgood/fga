...
Now let $S'\to S$ be a base change morphisms, and set $X'=X\times_S S'$, and $\scr{F}'=\scr{F}\otimes_{\OO_S}\OO_{S'}$, so that $X'$ is a prescheme over $S'$ endowed with a quasi coherent module $\scr{F}'$, and we can consider $\Quot(\scr{F}'/X'/S')$.
\oldpage{221-11}
We set
$$
  \shQuot_{\scr{F}/X/S}(S') = \Quot(\scr{F}'/X'/S')
$$
...
$$
  \Quot(\scr{F}'/X'/S') \to \Quot(\scr{F}''/X''/S'')
$$
and so $\shQuot_{\scr{F}/X/S}(S')$ is a *contravariant functor in $S'$* (where $S'$ is a prescheme over $S$), with values in the categories of sets.
