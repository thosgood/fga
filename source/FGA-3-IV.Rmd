...
$$
  \scr{F}_{(r)} = \scr{F}/N_r
$$
so that the associated prime cycles of $\src{F}_{(r)}$ are exactly the associated prime cycles of $\scr{F}$ that are of dimension $\geq r$, and, in particular, $\scr{F}_{(r)}$ is equal to $\scr{F}$ if and only if the associated prime cycles of $\scr{F}$ are of dimension $\geq r$.
With this, we have:
...
- The Poincaré **(TO-DO: should this be Hilbert?)** polynomials $P_\scr{F}$ of the sheaves $\scr{F}$ whose class is in $E$ have coefficients _in degrees $\leq (s-1)$_ that are bounded.

Under these conditions, the sheaves $\scr{F}_{(s)}$ (for the $\scr{F}$ whose class is in $E$) form a bounded family.
Furthermore, the coefficients in degree $(s-2)$ of the $P_\scr{F}$ are **TO-DO: minorés**.
:::
