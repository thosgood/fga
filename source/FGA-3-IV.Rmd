...
c. $f'_*(\scr{H}(\nu+k))=\scr{S}'_k f'_*(\scr{H}(\nu))$ for $k\geq0$.

For this last condition, we suppose that $X$ is written as the homogeneous prime spectrum of a quasi-coherent positively-graded algebra $\scr{S}_\bullet$ over $S$ that is generated by $\scr{S}_1$, and we set $\scr{S}'=\scr{S}\otimes_{\OO_S}\OO_{S'}$, so that $X'$ is the homogeneous prime spectrum of $\scr{S}'$.
