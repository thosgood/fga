...
$$
and is thus bounded.
With this in mind, saying that the $n^{r-1}$ coefficient $P_\scr{F}(n)$ is bounded below (resp. bounded above) is equivalent to saying the same thing for the $P_\scr{F}(n-k)=P_{\scr{F}(-k)}(n)$.
...
It is bounded above if and only if $d$ is bounded above;
