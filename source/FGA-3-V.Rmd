...
We note that the study of the Hilbert polynomials $Q\in\mathbb{Q}[t]$ allows us, as in the case of Hilbert schemes ([@Gro1959, IV]), to give a decomposition of $\repPic_{X/S}$ as a disjoint sum of opens $\repPic_{X/S}^Q$, and it seems plausible that these opens are quasi-projective over $S$;
...
The result that we invoke from [@Gro1959, III] that allows us to pass to the effective quotient can also easily be deduced from the existence theorem for Hilbert schemes (cf. for example [-]^[*[Trans.]* The original again cites here the 1962 Mumford--Tate seminar.]).
...
for now, there is no written proof, except when $X$ has "lots of local sections" over $S$, for example if $X$ is separable over a complete local ring.
...
In particular, the theory of duality for abelian schemes, and notably the results of Cartier type, then become slightly more formal (cf. for example [-]^[*[Trans.]* The original again cites here the 1962 Mumford--Tate seminar.])
:::
...
we use the structure explained in [§3](#fga-3-v-section-3) of $\repPic_{X/S'}$ (which implies that every finite subset of a fibre of $\repPic_{X/S'}$ over $S$ is contained in an open affine) for the existence of $\prod_{S'/S}\repPic_{X/S'}$.
:::
...
Suppose further that, for all $s\in S$, the **!!TO-DO: $\Hom k(s)\to\HH^0(X_{i_s},\OO_{X_{i_s}})$!!** are bijective, for $i=1,2$.
...
If $X$ is a projective scheme, say regular, and $Y$ a hyperplane section, then we can consider the "infinitesimal neighbourhoods" $X_n$ of $Y$ of all orders, as well as the Picard schemes $\repPic_{X_n/k}$;
