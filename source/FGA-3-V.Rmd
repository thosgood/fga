...
::: {.eqref #fga-3-v-equation-plus title="" latex=""}
$$
  \shDiv_{X/S}
  \to \shPic_{X/S}
\tag{plus}
...
Given a coherent module $\scr{Q}$ on the locally Noetherian prescheme $S$, we can easily show that, for any given $s\in S$, $\scr{Q}$ is free at $s$ if and only if $\mathbb{P}(\scr{Q})$ is flat over $S$ at the points over $s$ (in which case it will even be simple over $S$ at the points over $s$);
