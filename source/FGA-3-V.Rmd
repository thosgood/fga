...
\oldpage{232-01}For every prescheme (more generally, every ringed space) $X$, we define the (*absolute*) *Picard group* of $X$, denoted by $\Pic(X)$, to be the group of isomorphism classes of invertible (i.e. locally isomorphic to $\OO_X$) modules on $X$.
...
$$
If $P$ is representable, it follows from the theory of descent ([FGA 3.I, B, Theorem 2](#fga-3-i-section-B.1-theorem-2)) that the diagram [(1.5)](#fga-3-v-equation-1.5) is exact for all $u\in(\scr{M})$.
...
\oldpage{232-03}(N.B. The set $\overline{\HH}^0$ thus defined is larger than the set $\HH^0(T'/T,P)$ introduced in [FGA 3.I, A.4.a](#fga-3-i-section-A.4.a)).
...
\oldpage{232-09}For a given invertible $\scr{L}$ on $X$, the set of divisors $D$ that define $\scr{L}$ is in bijective correspondence with the quotient set $\Gamma(X,\scr{L})^\times/\Gamma(X,\OO_X^\times)$, where $\Gamma(X,\scr{L})^\times$ denotes the subset of $\Gamma(X,\scr{L})$ consisting of sections that are nowhere zero divisors.
...
\oldpage{232-10}Then the functor $\shDiv_{X/S}$ is representable, and, more precisely, is represented by an open of $\repHilb_{X/S}$.
:::
...
Using the map $D\mapsto\scr{L}(D)$, we obtain a functorial homomorphism
$$
  \shDiv_{X/S}
  \to \shPic_{X/S}
\tag{+}
...
Consider first of all the case where $\xi$ is defined by an invertible module $\scr{L}$ on $X$.
...
Then the relative Cartier divisors $D$ on $X/S$ such that $\scr{L}(D)$ and $\scr{L}$ define the same element of $\Pic(X/S)=\shPic_{X/S}(S)$, i.e. by [(2.4)](#fga-3-v-corollary-2.4) such that $\scr{L}(D)$ and $\scr{L}$ are locally isomorphic over $S$, are in bijective correspondence with the sections of the quotient sheaf $f_*(\scr{L})^\times/\scr{O}_S^\times$.
This correspondence is compatible with base change.
General arguments of "Künneth" type from *loc. cit.*^[*[Trans.]* The original also cites here a seminar on algebraic geometry at Harvard University by Mumford and Tate in the spring of 1962, but the notes from this were never completed and so there is no real existing citation. This citation appears again throughout both this present talk and the next.] show that the property of $X/S$ and the flatness of $\scr{L}$ over $S$ imply the existence of a coherent module $\scr{Q}$ on $S$, defined up to unique isomorphism, and an isomorphism of sheaves
$$
  f_*(\scr{L})
  \simto \shHom_{\OO_X}(\scr{Q},\OO_S)
$$
and the formation of $\scr{Q}$ is furthermore compatible with base change.
Here $f_*(\scr{L})^\times$ denotes the subsheaf of sets of $f_*(\scr{L})$ whose sections over $U$ are the sections of $\scr{L}$ over $f^{-1}(U)$ that define relative Cartier divisors on $f^{-1}(U)/U$, i.e. that induces sections that are non-zero divisors on the $X_s$ (for $s\in U$).
\oldpage{232-11}Using the hypothesis that the fibres $X_s$ are integral, this simply implies that the induced sections on the fibres $X_s$ are not identically zero, or, in terms of local homomorphisms $\scr{Q}\to\OO_S$, that these homomorphisms are surjective (Nakayama).
This shows that the set of sections of $f_*(\scr{L})^\times/\OO_S^\times$ is in bijective correspondence with the set of *invertible quotient modules of $\scr{Q}$*, or, by the definition of the projective bundle $\mathbb{P}(\scr{Q})$ associated to the coherent module $\scr{Q}$ (cf. [@Gro1960a, V, 2]), with the set of sections of $\mathbb{P}(\scr{Q})$ over $S$.
...
Let $f\colon X\to S$ be a flat proper morphism with integral geometric fibres, with $S$ locally Noetherian, and let $\scr{L}$ be an invertible module on $X$.
For every $S'$ over $S$, let $T(S')$ be the set of relative divisors $D$ on $X\times_S S'/S'$ such that $\scr{L}(D)$ is locally isomorphic to $\scr{L}\otimes_{\OO_S}\OO_{S'}$ over $S'$ (i.e. such that $\scr{L}(D)$ and $\scr{L}\otimes_{\OO_S}\OO_{S'}$) define the same element of $\Pic(X\times_S S'/S')$.
Then there exists a coherent module $\scr{Q}$ on $S$, determined up to unique isomorphism, such that the functor $T$ is represented by the projective bundle $\mathbb{P}(\scr{Q})$.
:::
...
::: {.rmenv #fga-3-v-remark-4.5 title="4.5" latex="{4.5}"}
Under the conditions of [(4.3)](#fga-3-v-theorem-4.3), the module $\scr{Q}$ on $S$ is not in general locally free, as we can see by the fact that the dimension of the reduced fibres of $\scr{Q}$, i.e. those of $\HH^0(X_s,\scr{L}_s)$ for varying $s\in S$, can *jump*.
Given a coherent module $\scr{Q}$ on the locally Noetherian prescheme $S$, we can easily show that, for any given $s\in S$, $\scr{Q}$ is free at $s$ if and only if $\mathbb{P}(\scr{Q})$ is flat over $S$ at the points over $s$ (in which case it is even simple over $S$ at the points over $s$);
when this happens, with $\scr{Q}$ defined in terms of $\scr{L}$ as above, this also implies that forming the direct image $f_*(\scr{L})$ *"commutes with base change on a neighbourhood of $s$"*, or that $f_*(\scr{L})_s\to\HH^0(X_s,\scr{L}_s)$ is surjective.
\oldpage{232-12}This will be the case if, for example, $\HH^1(X_s,\scr{L}_s)=0$.
...
For brevity, let $\scr{P}(S')=\Pic(X\times_S S'/S')$, and suppose, for simplicity, that $X/S$ admits a section.
Let $\scr{P}^+(S')$ be the subset of $\scr{P}(S')$ consisting of classes of the $\scr{L}$ that are invertible on $X\times_S S'$ such that
$$
  \begin{aligned}
    \RR^i f'_*(\scr{L}(n)) &= 0
    \qquad\text{for }i>0\text{ and all }n\geq0
  \\f'_*(\scr{L}(n)) &\neq0
    \qquad\text{for all }n\geq0.
  \end{aligned}
$$
These conditions are stable under base change, and thus define a subfunctor $\scr{P}^+$ of $\scr{P}$ that is evidently stable under translation by $\xi$.
Using Serre's "Theorems A and B" ([@GD1960, III, 2]) and generalities ([@Gro1960a, IV, 5]), we easily see that $\scr{P}$ is representable if and only if $\scr{P}^+$ is, and so $\scr{P}^+$ will be representable by an open $U$ of the prescheme $\repPic_{X/S}$ that represents $\scr{P}$, and the latter will be an increasing union of opens $U\setminus n\xi$.

For brevity, let $\scr{D}=\shDiv_{X/S}$, and let $\scr{D}^+$ be the inverse image of $\scr{P}^+$ under the canonical morphism $\scr{D}\to\scr{P}$.
...
\tag{+}
$$
and we already know that $\scr{D}^+$ is representable by an open $D^+$ of the prescheme $D=\repDiv_{X/S}$ (and, more precisely, by *projective bundles associated to locally free modules* that are everywhere non-zero);
this is due to the fact that, if $\scr{L}$ on $X\times_S S'$ is, as at the start of this section, then $f'_*(\scr{L})$ is a *locally free* non-zero module, whose formation commutes with base change;
with the notation of [(4.3)](#fga-3-v-theorem-4.3), $\scr{Q}$ is then isomorphic to the dual of $f'_*(\scr{L})$.
Using general criteria ([@Gro1960a, IV, 4.7]), we can thus conclude that $P^+$ is representable.
In *loc. cit.*, we take $\cal{S}$ to be the set of faithfully flat quasi-compact morphisms of preschemes (which are indeed effective epimorphisms, by [FGA 3.I, B](#fga-3-i-section-B.1)).
\oldpage{232-13}Condition (a) of *loc. cit.*, namely that $(+)$ is representable by morphisms that are elements of $\cal{S}$, is satisfied as we have just seen;
condition (b) says that the functor $\scr{P}^+$ is is compatible with faithfully flat quasi-compact descent, which is immediate.
It remains only to prove condition (c) of *loc. cit.*, namely that the equivalence $R$ in the prescheme $D^+$ induced by the $\cal{S}$-representable morphism ($+$) is $\cal{S}$-effective, i.e. is effective and such that $D^+\to D^+/R$ is in $\cal{S}$.
For this, note first of all that the opens ${D^+}^Q$ of $D^+$ that correspond to the virtual Hilbert polynomials $Q\in\mathbb{Q}[t]$ are stable under $R$ (since the fibres of $R$ are connected), which reduces the problem to proving that, for all $Q$, the induced equivalence relation $R^Q$ on ${D^+}^Q$ is $\cal{S}$-effective.
...
In the general case where $X/S$ does not necessarily admit a section, we can easily reduce to the above case by the technique of descent, where we can repeat the above proof with the modification that is imposed upon the definition of $\scr{P}^+$.
...
\oldpage{232-14}In this method, the passage to the quotient by a *proper* and flat equivalence relation in an open of the scheme of divisors is replaced by the passage to the quotient by the projective group in the scheme of immersions of $X$ into $\mathbb{P}_S^r$.
...
Applying a descent procedure gives a relative existence theorem, whose scope depends on the solution to questions about non-flat descent that were raised in [FGA 3.I, A.3.c](#fga-3-i-section-A.3.c), and of which we content ourselves here to explain only a particular case:

::: {.itenv #fga-3-v-proposition-6.3 title="Proposition 6.3" latex="{Proposition 6.3}"}
Let $f\colon X\to S$ be a proper morphism, and let $X_1$ and $X_2$ be subpreschemes of $X$ that are flat over $S$, defined by coherent ideals $\scr{J}_1$ and $\scr{J}_2$ (respectively) such that $\scr{J}_1\cap\scr{J}_2=(0)$ and such that $\OO_X/(\scr{J}_1+\scr{J}_2)$ is flat over $S$ (i.e. the subprescheme of $X$ that is the sup of $X_1$ and $X_2$ is $X$ itself, whereas their inf $Z$ is flat over $X$).
...
\oldpage{232-17}The proof then consists of noting that the data of a "rigidified" invertible module $\scr{L}$ on $X$ is equivalent to the data of a triple $(\scr{L}_1,\scr{L}_2,u)$, where $\scr{L}_i$ is a "rigidified" module on $X_i$, and $u$ is an isomorphism from $\scr{L}_1|Z$ to $\scr{L}_2|Z$ that is compatible with the rigidifications.
It remains only to verify that, for $\scr{L}_1$ and $\scr{L}_2$ fixed, the data of $u$ can be expressed as a section of a suitable scheme over $S$ that is affine over $S$, which is easy.
:::
...
Combining this with [(6.4)](#fga-3-v-corollary-3.4), we easily conclude:
...
\oldpage{232-18}It would thus suffice to show that the corresponding functorial morphism $\shPic_{X/k}\to\shPic_{X'/k}$ is representable (and, probably, representable by affine morphisms), since we already know that $\shPic_{X'/k}$ is representable.
...
when $X$ is irreducible of dimension $\geq4$ (resp. $\geq3$), the canonical morphism
$$
  \repPic_{X/k}
  \to\repPic_{X_n/k}
