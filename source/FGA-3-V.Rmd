...
\oldpage{232-03}
(N.B. The set $\overline{\HH}^0$ thus defined is larger than the set $\HH^0(T'/T,P)$ introduced in [FGA 3.I, A.4.a](FGA-3-I.html#fga-3-i-section-A.4.a)).
...
\oldpage{232-11}
Using the hypothesis that the fibres $X_s$ are integral, this simply implies that the induced sections on the fibres $X_s$ are not identically zero, or, in terms of local homomorphisms $\scr{Q}\to\OO_S$, that these homomorphisms are surjective (Nakayama).
...
\oldpage{232-12}
This will be the case if, for example, $\HH^1(X_s,\scr{L}_s)=0$.
...
\oldpage{232-13}
Condition (a) of *loc. cit.*, namely that $(+)$ is representable by morphisms that are elements of $\cal{S}$, is satisfied as we have just seen;
...
\oldpage{232-17}
The proof then consists of noting that the data of a "rigidified" invertible module $\scr{L}$ on $X$ is equivalent to the data of a triple $(\scr{L}_1,\scr{L}_2,u)$, where $\scr{L}_i$ is a "rigidified" module on $X_i$, and $u$ is an isomorphism from $\scr{L}_1|Z$ to $\scr{L}_2|Z$ that is compatible with the rigidifications.
