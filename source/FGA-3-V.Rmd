...
In *loc. cit.*, we take $\scr{S}$ to be the set of faithfully flat quasi-compact morphisms of preschemes (which are indeed effective epimorphisms, by [@Gro1959, I, B]).
\oldpage{232-13}
Condition (a) of *loc. cit.*, namely that $(+)$ is representable by morphisms that are elements of $\scr{S}$, is satisfied as we have just seen;
condition (b) says that the functor $\scr{P}^+$ is is compatible with faithfully flat quasi-compact descent, which is immediate.
It remains only to prove condition (c) of *loc. cit.*, namely that the equivalence $R$ in the prescheme $D^+$ induced by the $\scr{S}$-representable morphism ($+$) is $\scr{S}$-effective, i.e. is effective and such that $D^+\to D^+/R$ is in $\scr{S}$.
For this, note first of all that the opens ${D^+}^Q$ of $D^+$ that correspond to the virtual Hilbert polynomials $Q\in\mathbb{Q}[t]$ are stable under $R$ (since the fibres of $R$ are connected), which reduces the problem to proving that, for all $Q$, the induced equivalence relation $R^Q$ on ${D^+}^Q$ is $\scr{S}$-effective.
...
The result that we invoke from [@Gro1959, III] that allows us to pass to the effective quotient can also easily be deduced from the existence theorem for Hilbert schemes (cf. for example [-]^[*[Trans.]* The original again cites here the 1962 seminar on algebraic geometry at Harvard University by Mumford and Tate.]).
