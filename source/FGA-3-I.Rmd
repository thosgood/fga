...
The applications of [Theorem 1](#FGA-3-I-theorem-1), [Theorem 2](#FGA-3-I-theorem-2), and [Theorem 3](#FGA-3-I-theorem-3) below (in \hyperref[B.1]{(B.1)}) are, however, already vast in number. 
...
We make clear to point out, however, that the preschemes considered in this current exposé are not necessarily Noetherian, and that the
\oldpage{190-02}
...
::: {.rmenv #FGA-3-I-section-A.1-definition-1 title="Definition 1.1" latex="{Definition 1.1}"}
...
\label{definition:A.1.2}
  ...
  $$
    ...
    \end{tikzcd}
  $$
  of maps of sets is said to be _exact_ if $u$ is a bijection from $E$ to the subset of $E'$ consisting of the $x'\in E'$ such that $v_1(x')=v_2(x')$.
\end{rmenv}
...
\label{definition:A.1.3}
  ...
  this diagram is said to be _$\cal{F}$-exact_ if, for every pair $(\xi,\eta)$ of elements of $\cal{F}_S$, the diagram
  $$
...
In the case where the diagram \hyperref[equation-definition:A.1.3]{(+)} is $\cal{F}$-exact, the object $\xi$ in \hyperref[definition:A.1.5]{Definition 1.5} is then determined up to unique isomorphism, and _the category $\cal{F}_S$ is equivalent to the category of objects of $\cal{F}_{S'}$ endowed with effective gluing data_. -->
...
\label{definition:A.1.7}
  A morphism $\alpha\colon S'\to S$ is said to be an _$\cal{F}$-descent morphism_ if the diagram
  $$
    \begin{tikzcd}
      S
      & S' \lar[swap,"\alpha"]
  ...
  we say that $\alpha$ is a _strict $\cal{F}$-descent morphism_ if, further, every descent data (\hyperref[definition:A.1.6]{Definition 1.6}) on any object of $\cal{F}_{S'}$ is effective.
...
\label{definition:A.2.4}
  A morphism $\alpha\colon S'\to S$ is said to be a _descent morphism_ (resp. a _strict descent morphism_) if it is an $\cal{F}$-descent morphism (resp. a strict $\cal{F}$-descent morphism) (cf. \hyperref[definition:A.1.7]{Definition 1.7}), where $\cal{F}$ denotes the fibred category over $\cal{C}$ of objects of $\cal{C}$ over objects of $\cal{C}$ (\hyperref[example:A.1.1(1)]{Example 1}).
\end{rmenv}
...
$$
_is an isomorphism for every pair of elements $\xi,\eta\in\cal{F}_S$ implies that $\alpha\colon S'\to S$ is an $\cal{F}$-descent morphism_ (\hyperref[definition:A.1.7]{Definition 1.7}).
...
\oldpage{190-14}
groups.
With this setup, we claim that _$Z^1(S'/S,G)$ is canonically identified with the set of descent data on $\xi'=\xi\times_S S'$ with respect to $S'\to S$_ (\hyperref[definition:A.1.6]{Definition 1.6}), and that _$\HH^1(S'/S,G)$ can be identified with the set of isomorphism classes of objects of $\cal{F}_{S'}$ endowed with a descent data relative to $\alpha\colon S'\to S$ that are isomorphic, as objects of $\cal{F}_{S'}$, to $\xi'=\xi\times_S S'$_.
...
\label{theorem:B.1(1)}
  ...
  Then $\alpha$ is a _strict descent morphism_ (cf. \hyperref[definition:A.1.7]{A, Definition 1.7}) for the fibred category $\cal{F}$ of quasi-coherent sheaves (cf. \hyperref[example:A.1.1(2)]{A, Example 2}).
\end{itenv}
...
\oldpage{190-18}
  $$
    ...
    \end{tikzcd}
  $$
  of maps of sets is _exact_ (cf. \hyperref[definition:A.1.1]{A, Definition 1.1}).
\end{itenv}
...
\label{theorem:B.1(2)}
  ...
  Then $\alpha$ is a (non-strict, in general) _descent morphism_ (cf. \hyperref[definition:A.2.4]{A, Definition 2.4}), and it is a _strict descent morphism_ for the fibred category of affine schemes over preschemes (cf. \hyperref[definition:A.1.7]{A, Definition 1.7}).
\end{itenv}
...
$$
  ...
  \end{tikzcd}
$$
of natural maps is _exact_, i.e. $\alpha^*$ is a bijection from $\Hom_S(X,Y)$ to the subset of $\Hom_{S'}(X',Y')$ consisting of homomorphisms that are compatible with the canonical descent data on $X'$ and $Y'$ (i.e. whose inverse images under the two projections from $S''$ to $S'$ are equal).
