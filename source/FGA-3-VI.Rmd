...
iii. Suppose that $A$ exists, and let $B=\Alb^0(X/S)$ be the dual abelian scheme (i.e. $B=\Pic_{A/S}^0$ [-]^[*[Trans.]* The original again cites here the 1962 Mumford--Tate seminar.]).
  Then we can canonically construct a principal homogeneous space $P=\Alp^1(X/S)$ for $B$, and an $S$-morphism $X\to P$ that is universal for the $S$-morphisms from $X$ to para-abelian schemes (i.e. to principal homogeneous spaces for abelian schemes).
...
i. This is a general property of rigidity for abelian subschemes of commutative group schemes: if two such subschemes agree set-theoretically at a point $s\in S$, then they agree over the entire connected component of $s$ ([-]^[*[Trans.]* The original again cites here the 1962 Mumford--Tate seminar.]).
