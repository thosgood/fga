...
Let $f\colon X\to S$ be a proper and simple morphism such that $\repPic_{X/S}$ exists and is the sum of the schemes $P^{(i)}$ of finite type over $S$ (cf. [FGA 3.V, Proposition 4.1](FGA-3-V.html#fga-3-v-proposition-4.1)).
...
But this latter space is exactly $\HH^1(P_0,\scr{G}_{P_0/k})\otimes V$, where $\scr{G}_{P_0/k}$ is the tangent sheaf to $P_0/k$, and thus also the space that expresses the indeterminacy that there was in the lifting of $P_n$ to $P_{n+1}$ ([FGA 3.II](FGA-3-II.html)).
...
For (i), we use, in an essential way, the ideas of non-flat descent (see [FGA 3.I, p.9](FGA-3-I.html#fga-3-i-section-A.2.c)).
