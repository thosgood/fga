...
Then there exists an invertible module $\scr{L}'$ on $X$ whose $n$-th tensor power is isomorphic to $\scr{L}$ (if $n$ is coprime to the residual characteristic of $k=k(\scr{L})$).
:::
...
ii. For there to exist such an abelian subscheme, it is necessary and sufficient that it exist after every base change $S'\to S$, where $S'$ is local Artinian;
  if $S$ is the spectrum of a local ring, it even suffices to test with the $S'=\Spec(A_n)$ where $A_n=A/\frak{m}^{n+1}$.
...
Note that, for this, it suffices to construct an abelian scheme $P_{n+1}$ over $A_{n+1}$ that extends $P_n=\repPic_{X_n/A_n}^0$, along with an invertible module $\scr{L}_{n+1}$ on $X_{n+1}\times_{A_{n+1}}P_{n+1}$ that extends the invertible module $\scr{L}_n$ on $X_n\times_{A_n}P_n$ that arises in the definition of the Picard scheme $\repPic_{X_n/A_n}$ as the solution to a universal problem.
(N.B. We can suppose that $X$ is endowed with a section over $S$...).
For this construction, we must use the following key result: *every abelian scheme defined over a quotient of an Artinian local ring can be extended* (in other words, the absolute "formal scheme of modules" ([@Gro1959, II]) for an abelian scheme over an algebraically closed field is simple over the ring of Witt vectors over $k$);
...
we then find an obstruction to lifting $\scr{L}_n$, found in $\HH^2(X_0\times P_0,\OO_{X_0\times P_0})\otimes_k V$ (where $V=\frak{m}^{n+1}/\frak{m}^{n+2}$), and more precisely in the subspace $\HH^1(X_0,\OO_{X_0})\otimes\HH^1(P_0,\OO_{P_0})\otimes_k V$ (taking into account the fact that the restriction of $\scr{L}_n$ to the two factors $X_n$ and $P_n$ is trivial).
But this latter space is exactly $\HH^1(P_0,\scr{G}_{P_0/k})\otimes V$, where $\scr{G}_{P_0/k}$ is the tangent sheaf to $P_0/k$, and thus also the space that expresses the indeterminacy that there was in the lifting of $P_n$ to $P_{n+1}$ ([@Gro1959, II]).
So we can correct this lifting (in exactly one way, as should be the case) in such a way as to kill the obstruction to lifting $\scr{L}_n$.
:::
...
Note that these finiteness questions that we have just highlighted still make sense even without supposing the existence of $\repPic_{X/S}$, since they can be expressed by saying that certain families of invertible modules are "limited", in the sense of [@Gro1959, IV]:
...
Let $\OO_X(1)$ be a very ample module on $X$ with respect to $S$, and $E$ a subset of $\repPic_{X/S}$ corresponding to a family $(\scr{L}_i)$ of invertible modules on the geometric fibres $\overline{X}_{s_i}$ of $X/S$, with $D_i$ a (not-necessarily positive) divisor on $\overline{X}_{s_i}$ that defines $\scr{L}_i$, and let $a_n^{(i)}t^n+\ldots+a_0^{(i)}$ be the Hilbert polynomial of $\scr{L}_i$, and let $\xi=\xi_i$ be a divisor that defines $\OO_X(1)$, i.e. a hyperplane section.

Then the following conditions are equivalent:

a. $Q$ is quasi-compact, i.e. contains an open of finite type over $S$, i.e.. the family $(\scr{L}_i)$ is limited.
b. $E$ is contained in the union of a finite number of the sets $\repPic_{X/S}^Q$, for $Q\in\mathbb{Q}[t]$, i.e. the set of Hilbert polynomials of the $\scr{L}_i$ is finite.
c. (If the fibres of $X/S$ are all of the same dimension $n$).^[*[Trans.]* Conditions (c) and (d) were originally called (b') and (b'').]
  The coefficients $a_{n-1}^{(i)}$ and $a_{n-2}^{(i)}$ of the Hilbert polynomials of the $\scr{L}_i$ are contained in a finite set.
...
ii. \oldpage{C-08}Suppose that $Y$ is projective over $S$, endowed with an invertible module that is ample with respect to $S$, and let $X$ be the scheme of zeros of an arbitrary section of this module.
  ...
  Finally, suppose that the irreducible components of the fibres of $Y/S$ are of dimension $\geq3$.
...
We thus conclude that an invertible module $\scr{L}$ on $Y$ is $\tau$-equivalent to $0$ if and only if its inverse image in $X$ is;
...
Generally, the conjunction of (i) for a finite morphism and (ii) allows us to reduce, for the majority of finiteness questions, to the case where $X/S$ has integral and normal geometric fibres of dimension $\leq2$;
...
vii. Let $X$ be a proper scheme over an algebraically closed field.
  Then there exists a finite number of integral closed curves $C_i$ (for $1\leq i\leq r$) in $X$, such that the following property is satisfied:
  for every subset $M$ of $\repPic_{X/k}$, $M$ is quasi-compact if and only if the integers $\deg\scr{L}_{C'_i}$ (for $\scr{L}\in M$) are bounded (where $C'_i$ denotes the normalisation of $C_i$).
...
viii. Let $X/k$ be a proper scheme over a field, and $\scr{L}$ an invertible module on $X$.
  Then the following conditions are equivalent:

  a. $\scr{L}$ is $\tau$-equivalent to $0$.
  b. For every coherent module $F$, we have $\chi(F\otimes\scr{L})=\chi(F)$, where $\chi$ denotes the Euler--Poincaré characteristic (or, condition (b'), we can simply take $F=\OO_Y$, where $Y$ is an integral closed subscheme of dimension $1$ in $X$).
  c. For every $Y$ as above, writing $Y'$ to mean the normalised curve, we have $\deg\scr{L}_{Y'}=0$.
  ...
  d. For every integer $m$, $\scr{L}^{\otimes m}(1)$ is ample.
  e. (If $X$ is integral).
    For every pair of integers $(m,n)$, we have
    $$
      \chi(\scr{L}^{\otimes m}(n))
      = \chi(\OO(n))
    $$
    \oldpage{C-10}(i.e. (b) is true for $F=\scr{L}^{\otimes m}(n)$).

  For the sufficiency of this last condition, note that it implies that the Hilbert polynomials of the $\scr{L}^{\otimes m}$ are all equal, and thus, by the Mumford criteria (iii), the $\scr{L}^{\otimes m}$ remain in a quasi-compact subset of $\repPic_{X/k}$, i.e. we have (a).
...
For (i), we use, in an essential way, the ideas of non-flat descent (see [@Gro1959, I, p.9]).
One finds that (thinking only of finiteness results) the lack of effectivity criteria for descent data is inoffensive.
Mumford has recently proven a slightly weaker form of (iii), where the criteria makes use of *all* the coefficients of the Hilbert polynomial.
...
From the dimension hypothesis it then follows that the geometric fibres of $Y$ and of $X$ are of depth $\geq2$ at their closed points, which allows us to apply the "equivalence criteria" under the form that is given in [@Gro1960b, XII], and finishes the proof of (ii).
