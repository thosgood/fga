...
iii. $F$ commutes with finite products, and, for every exact diagram
  ...
  $$
  in $\cal{C}$ (cf. [FGA 3.I, A, Definition 2.1](FGA-3-I.html#fga-3-i-section-A.2-definition-2.1)), the image of the diagram under $F$
  $$
...
\oldpage{195-06}We say that $\xi$ (or the pair $(X,\xi)$) is _minimal_ if, for all $X'\in\cal{C}$ and all $\xi'\in F(X')$, and for every strict monomorphism (cf. [FGA 3.I, A, §2](FGA-3-I.html#fga-3-i-section-A.2)) $u\colon X'\to X$ such that $\xi=F(u)(\xi')$, $u$ is an isomorphism.
...
In the current state of the technique of descent (cf. the questions asked in [FGA 3.I, p.9](FGA-3-I.html#fga-3-i-section-A.2.c)), this criterion is not directly verifiable, in this form, in the most important cases, and we need criteria that seem less demanding.
...
ii. for every algebra $A\in\cal{C}$ and every homomorphism $A\to A'$ in $\cal{C}$ such that the diagram
  ...
  $$
  is exact (cf. [FGA 3.I, A, Definition 1.2](FGA-3-I.html#fga-3-i-section-A.1-definition-1.2)), the diagram
  $$
...
The verification of condition (ii) splits into two cases: case (a), where $A'$ is a free $A$-module, can be dealt with using the _technique of descent by flat morphisms_ (cf. [FGA 1, Theorems 1, 2, and 3](FGA-1.html)), which offers no difficulty;
...
Let $\cal{F}$ be the fibred category (cf. [FGA 3.I, A, Definition 1.1](FGA-3-I.html#fga-3-i-section-A.1-definition-1.1)) of quasi-coherent sheaves that are flat over varying preschemes.
Then the morphism $\Spec(A')\to\Spec(A)$ is a _strict $\cal{F}$-descent morphism_ (cf. [FGA 3.I, A, Definition 1.7](FGA-3-I.html#fga-3-i-section-A.1-definition-1.7)).
:::
...
$$
(cf. [FGA 3.I, A.4.e](FGA-3-I.html#fga-3-i-section-A.4.e)), with the hypothesis that $\frak{m}A'\subset A$ allowing us to easily reduce to the case where $A$ is a field (namely $A/\frak{m}$).
We can then apply the equivalences described in [FGA 3.I, p.16](FGA-3-I.html#fga-3-i-section-A.4.e).
...
This condition naturally arises in the application of the technique of descent, _in eliminating the automorphisms of an invertible sheaf $\scr{L}$ on $X$ by endowing them with a marked section over the section $s$_ ([FGA 3.I, B.4](FGA-3-I.html#fga-3-i-section-B.4)).
