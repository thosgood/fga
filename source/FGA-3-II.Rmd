...
## (A.1) Representable functors {#fga-3-ii-section-A.1}

Let $\cal{C}$ be a category.
\oldpage{195-01}For all $X\in\cal{C}$, let $h_X$ be the contravariant functor from $\cal{C}$ to the category $\Set$ of sets given by
$$
  \begin{aligned}
...
## (A.2) Pro-representable functors, pro-objects {#fga-3-ii-section-A.2}
...
\oldpage{195-04}Of course, an object $X$ of $\cal{C}$ canonically defines a pro-object, denoted again by $X$, so that _$\cal{C}$ is equivalent to a full subcategory of $\Pro(\cal{C})$_.
...
\oldpage{195-05}Under these conditions, the projective system $(X_i)_{i\in I}$ is determined _up to unique isomorphism_ (in the usual sense of isomorphisms of projective systems).
...
## (A.3) Characterisation of pro-representable functors {#fga-3-ii-section-A.3}
...
iii. $F$ commutes with finite products, and, for every exact diagram
  ...
  $$
  in $\cal{C}$ (cf. [FGA 3.I, A, Definition 2.1](#fga-3-i-section-A.2-definition-2.1)), the image of the diagram under $F$
  $$
...
\oldpage{195-06}We say that $\xi$ (or the pair $(X,\xi)$) is _minimal_ if, for all $X'\in\cal{C}$ and all $\xi'\in F(X')$, and for every strict monomorphism (cf. [FGA 3.I, A, §2](#fga-3-i-section-A.2)) $u\colon X'\to X$ such that $\xi=F(u)(\xi')$, $u$ is an isomorphism.
...
## (A.4) Example: groups of Galois type, pro-algebraic groups {#fga-3-ii-section-A.4}

If $\cal{C}$ is the category of ordinary finite groups, then $\Pro(\cal{C})$ is equivalent to the category of totally disconnected compact topological^[*[Trans.]* Here the word "Hausdorff" is implicit.] groups.
...
## (A.5) Example: "formal varieties" {#fga-3-ii-section-A.5}
...
\oldpage{195-07}The conditions of the corollary to [Proposition 3.1](#fga-3-ii-section-A.3-proposition-3.1) are then satisfied.
...
For the corresponding local component $O_\xi$ of $O$ to be a _Noetherian_ ring, it is necessary and sufficient that the set $F(I_K,\xi)$ of elements of $F(I_K)$ that are reducible along $\xi$ be a vector space of _finite dimension_ over $K$.
...
*[Comp.]* The formula given above is only correct when $\Lambda$ is a field; in the general case, we must replace $\frak{m}_\xi/\frak{m}_\xi^2$ with the quotient of this space by the image of $\frak{n}_\xi/\frak{n}_\xi^2$, where $\frak{n}$ is the maximal ideal of $\Lambda$.
:::
...
(*[Comp.]* The following definition is correct only when the residue extension $k'/k$ is _separable_; for the general case, see [@Gro1960b, III, 1.1].)
...
\oldpage{195-09}To finish, we say some words, in the above context, about _rings of definition_.
...
By the corollary of [Proposition 3.1](#fga-3-ii-section-A.3-proposition-3.1), to ensure this, it is necessary and sufficient that $F$ be _left exact_.
In the current state of the technique of descent (cf. the questions asked in [FGA 3.I, p.9](#fga-3-i-section-A.2.c)), this criterion is not directly verifiable, in this form, in the most important cases, and we need criteria that seem less demanding.
...
ii. for every algebra $A\in\cal{C}$ and every homomorphism $A\to A'$ in $\cal{C}$ such that the diagram
  ...
  $$
  is exact (cf. [FGA 3.I, A, Definition 1.2](#fga-3-i-section-A.1-definition-1.2)), the diagram
  $$
...
The verification of condition (ii) splits into two cases: case (a), where $A'$ is a free $A$-module, can be dealt with using the _technique of descent by flat morphisms_ (cf. [FGA 1, Theorems 1, 2, and 3](#fga-1)), which offers no difficulty;
...
Let $\cal{F}$ be the fibred category (cf. [FGA 3.I, A, Definition 1.1](#fga-3-i-section-A.1-definition-1.1)) of quasi-coherent sheaves that are flat over varying preschemes.
Then the morphism $\Spec(A')\to\Spec(A)$ is a _strict $\cal{F}$-descent morphism_ (cf. [FGA 3.I, A, Definition 1.7](#fga-3-i-section-A.1-definition-1.7)).
:::
...
$$
(cf. [FGA 3.I, A.4.e](#fga-3-i-section-A.4.e)), with the hypothesis that $\frak{m}A'\subset A$ allowing us to easily reduce to the case where $A$ is a field (namely $A/\frak{m}$).
We can then apply the equivalences described in [FGA 3.I, p.16](#fga-3-i-section-A.4.e).


## **C.** Applications to some particular cases {-}


## (C.1) General remarks on functors represented by preschemes {#fga-3-ii-section-C.1}

Let $S$ be a locally Noetherian prescheme.
\oldpage{195-11}A prescheme $X$ over $S$ is said to be _locally of finite type_ over $S$ if, for all $x\in X$ that project to $y\in Y$, there exists an affine neighbourhood of $y$ of ring $A$, and an affine neighbourhood of $x$ (over the aforementioned affine neighbourhood of $y$) of ring $B$, such that $B$ is an $A$-algebra of finite type.
...
## (C.2) The schemes $\repHom_S(X,Y)$, $\prod_{X/S}Z$, $\repAut(X)$, etc. {#fga-3-ii-section-C.2}
...
Then [Theorem 1](#fga-3-ii-section-B-theorem-1) and [Theorem 2](#fga-3-ii-section-B-theorem-2) give:
...
\oldpage{195-15}More generally, we can start with a quasi-coherent sheaf $\scr{G}$ on $X$, and take $F(T)$ to be the set of quotient sheaves of $\scr{G}_T$ that are flat over $T$.
...
## (C.3) Picard schemes {#fga-3-ii-section-C.3}

*[Comp.]* For a more complete study, see [FGA 3.V](FGA-3-V.html).
...
This condition naturally arises in the application of the technique of descent, _in eliminating the automorphisms of an invertible sheaf $\scr{L}$ on $X$ by endowing them with a marked section over the section $s$_ ([FGA 3.I, B.4](#fga-3-i-section-B.4)).
...
\oldpage{195-17}_The golden rule to remember, in the context of the current_
...
## (C.4) Formal modules of a variety {#fga-3-ii-section-C.4}
...
::: {.eqenv #fga-3-ii-section-C.4-equation-asterisk}
$$
...
(From here, using the "existence theorem" of [@Gro1958a] to pass to the case where $A$ is complete local Noetherian, and then the technique of descent from [FGA 3.I](FGA-3-I.html) for the general case, we can prove the analogous claim for all locally Noetherian connected $S$).
...
## (C.5) Extension of coverings {#fga-3-ii-section-C.5}
...
(*[Comp.]* It is also necessary to assume that the section defining $U$ is not a zero divisor not only on $\frak{X}$, but also on every $X_n$.)
...
\oldpage{195-20}We start, in particular, with a flat covering $X'_0$ of $X_0$, unramified over $U_0$, and let $G(\frak{X})$ be the set of classes, up to isomorphism (inducing the identity on $X'_0$), of flat coverings $\frak{X}'$ of $\frak{X}$ that induce $X'_0$ on $X_0$ (and that are thus necessarily unramified over $U$).
...
With this, the results of [@Gro1958a] and [FGA 3.I](FGA-3-I.html) imply, first of all, the following results:
...
$$
\oldpage{195-21}where $\scr{O}$ is a topological $\Lambda$ algebra of the type described in [§A.5](#fga-3-ii-section-A.5), or that this is true when we restrict to only Artinian algebras $A$.
