...
- (b<sub>s</sub>) The Poincaré polynomials $P_\sh{F}$ of the sheaves $\sh{F}$ whose class is in $E$ have coefficients _in degrees $\leqslant (s-1)$_ that are bounded.
...
\oldpage{221-09}These $s_i$ generate $\OO_{\PP_K^r}(d)$ at the points of $U$, and are thus not all zero, which implies that $d\geqslant0$.
...
It then follows from the Künneth relations [@GD1960, III, §7] and from (b) that the conditions (a) and (b) are stable under base change, and imply that, for $n\geqslant\nu$, forming $f'_*(\sh{G}(n))$ and $F'_*(\sh{H}(n))$ commutes with extension of the base (*loc. cit.*).
...
for (a) and (b), this follows in fact from the "Theorem of holomorphic functions" [@GD1960, III, §7], and (c) follows from the Nakayama lemma and the fact that we know that $f'_*(\sh{H}(n+k))=S'_kf'_*(\sh{H}(n))$ anyway for $n$ large enough and $k\geqslant0$ [@GD1960, III,§2].
