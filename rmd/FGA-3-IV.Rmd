...
\providecommand{\FXS}{{\sh{F}/X/S}}
\providecommand{\FXSp}{{\sh{F}'/X'/S'}}
...
\oldpage{221-01}The techniques described in [FGA 3.I](FGA-3-I.html) and [FGA 3.II](FGA-3-II.html) were, for the most part, independent of any projective hypotheses on the schemes in question.
...
The techniques used are typically projective, and practically make no use of any results from [FGA 3.I](FGA-3-I.html) and [FGA 3.II](FGA-3-II.html).
Here we will construct "Hilbert schemes", which are meant to replace the use of Chow coordinates, as was mentioned in [FGA 3.II, C.2](#fga-3-ii-section-C.2).
In the next article, the theory of passing to the quotient in schemes, developed in [FGA 3.III](FGA-3-III.html), combined with the theory of Hilbert schemes, will allow us, for example, to construct Picard schemes (defined in [FGA 3.II, C.3](#fga-3-ii-section-C.3)) under rather general conditions.

In summary, we can say that we now have a more or less satisfying technique of projective constructions, apart from the fact that we are still missing^[See the addendum at the end of this article.] a theory of passing to the quotient by groups such as the projective group, acting "without fixed points" (cf. [FGA 3.III, §8](#fga-3-iii-section-8)).
...
\oldpage{221-02}So, if $K$ and $K'$ are arbitrary extensions of $k$, and $\sh{F}$ a quasi-coherent sheaf on $X_K$ and $\sh{F}'$ a quasi-coherent sheaf on $X_{K'}$, then we say that $\sh{F}$ and $\sh{F}'$ are _equivalent_ if there exists an extension $K''/k$ as well as $k$-homomorphisms $K\to K''$ and $K'\to K''$ such that $\sh{F}_{K''}$ and $\sh{F}'_{K''}$ are isomorphic over $X_{K''}$.
...
Consider the classes of all sheaves of the form $\sh{F}\otimes\sh{F}'$, where $\sh{F}$ and $\sh{F}'$ are coherent sheaves on the _same_ $X_K$, with the class of $\sh{F}$ being in $E$ and the class of $\sh{F}'$ being in $E'$.
...
Our aim in this section is to give a definition of certain sets of classes of sheaves, said to be _bounded_, and to show that the most standard operations $\sh{U}$, applied to bounded sets, give sets that are again bounded.
...
We say that $E$ is _bounded_ if there exists a prescheme $S'$ of finite type over $S$, along with a coherent sheaf $\sh{F}'$ on $X'=X\times_S S'$, such that $E$ is contained in the set of classes of sheaves on the fibres of $X/S$ defined by $\sh{F}'$.
:::
...
a bounded family $E$ is always bounded by an algebraic family defined by a coherent sheaf $\sh{F}$ on some $X_T$ (with $T$ of finite type over $S$) that is _flat_ with respect to $T$.
(We "cut into bits" the initial space of parameters).
\oldpage{221-04}Such flatness properties on suitable sheaves indeed ensure the commutativity of operations such as $\shTor_i(\sh{F},\sh{F}')$ with arbitrary base change.
...
i. the family of kernels, cokernels, and images of homomorphisms $\sh{F}\to\sh{F}'$ (where the class of $\sh{F}$ is in $E$ and the class of $\sh{F}'$ is in $E'$) is bounded;
ii. the family of extensions $\sh{F}''$ of $\sh{F}$ by $\sh{F}'$ (where the class of $\sh{F}$ is in $E$ and the class of $\sh{F}'$ is in $E'$) is bounded.
:::
...
\oldpage{221-05}This is also true for the kernel, cokernel, and image of the above homomorphism, and the consideration of this proves (i) (provided that we assume the cokernel to be flat with respect to $T_0$, in which case we can again reduce to cutting $T_0$ into pieces...).
:::
...
We can prove the analogous result to [(1.3)](#fga-3-iv-proposition-1.3) for the sheaves canonically induced from $\sh{F}$ by the theory of primary decomposition;
...
or the $\OO_{X_K}/\mathfrak{p}$, where $\mathfrak{p}$ is a prime sheaf of ideals associated to $\sh{F}$, or the $\OO_{X_K}/\mathfrak{q}$, where the $\sh{q}$ is a primary sheaf of ideals associated to a component of the support of $\sh{F}$ (the reference field being algebraically closed).
...
$$
  P_\sh{F}(n) = \text{the Euler--Poincaré characteristic of }\sh{F}(n)\text{ on }X_k
$$
which is a polynomial in the integer $n$, and called the _Hilbert polynomial_ of $\sh{F}$.
...
Now, if $\sh{F}$ is a coherent sheaf on $X$ which is _flat_ with respect to $S$, then the Hilbert polynomials of the sheaves $\sh{F}_S$ induced on the fibres $X_S$ (with respect to one single connected component of $S$) are all equal [@GD1960, III, §7].
...
\oldpage{221-06}Recall also that, if $\sh{F}$ is a coherent sheaf on $X$, then it is isomorphic to a quotient sheaf of a sheaf of the form $\OO_X(-n)^N$, for some large enough $n,N$.
...
a. There exists a coherent sheaf $\sh{L}$ on $X$ (which we can suppose to be of the form $\OO_X(-n)^N$) such that $E$ is contained in the family of classes of coherent sheaves given by quotients of sheaves of the form $\sh{L}_K$;
b. The Hilbert polynomials $P_\sh{F}$ of the sheaves $\sh{F}$ whose class is in $E$ are elements of a single _finite_ set of polynomials.
:::
...
We thus have that $N_r=\sh{F}$ for $r>\dim\supp\sh{F}$, and $N_r=0$ for $r\leq0$, and we thus obtain a finite increasing filtration of $\sh{F}$ whose factors $N_r/N_{r+1}$ are such that their associated prime cycles are exactly the associated prime cycles of $\sh{F}$ that are of dimension $r$.
We set
$$
  \sh{F}_{(r)} = \sh{F}/N_r
$$
so that the associated prime cycles of $\sh{F}_{(r)}$ are exactly the associated prime cycles of $\sh{F}$ that are of dimension $\geq r$, and, in particular, $\sh{F}_{(r)}$ is equal to $\sh{F}$ if and only if the associated prime cycles of $\sh{F}$ are of dimension $\geq r$.
With this, we have:
...
- (b<sub>s</sub>) The Poincaré polynomials $P_\sh{F}$ of the sheaves $\sh{F}$ whose class is in $E$ have coefficients _in degrees $\leq (s-1)$_ that are bounded.
...
Suppose that the sheaves $\sh{F}$ whose class is in $E$ are such that all their associated prime cycles are of dimension $d$, with $s\leq d\leq r$.
Then, in condition (b) of [Theorem 2.1](#fga-3-iv-theorem-2.1), we can restrict to the coefficients of $P_\sh{F}$ between degree $(s-1)$ and $r$, inclusive.
:::
...
$$
  P_\sh{F}(n) = a_\sh{F}n^r/r! + b_\sh{F}n^{r-1}/(r-1)! + \text{terms of degree }<r-1.
$$
...
::: {.proof}
By replacing $S$ by a union of subschemes of $S$ that cover $S$, we can suppose that there exists a *finite* morphism $f\colon X\to\PP_S^r$ such that $\OO_X(1)$ is isomorphic to the inverse image of $\OO_{\PP_S^r}(1)$, and thus, for every coherent sheaf $\sh{F}$ on $X$, we have that $P_\sh{F}=P_{f_*(\sh{F})}$.
...
$$
  f_*(\sh{F})_{(r)} = f_*(\sh{F}_{(r)}).
$$
\oldpage{221-08}This thus allows us to reduce to the case where $X=\PP_S^r$.
...
Since $\PP_K^r$ is normal, it follows that $\sh{F}$ is locally free of rank $a=a_\sh{F}$ on an open $U=\PP_K^r\setminus Y$, where $Y$ is of codimension $\geq2$.
Thus $\bigwedge^a\sh{F}$ is a sheaf on $\PP_K^r$ whose restriction to $U$ is invertible, and thus (since $\PP_K^r$ is regular, and $Y$ is of codimension $\geq2$) isomorphism to the restriction of an invertible sheaf on $\PP_K^r$, defined up to isomorphism.
...
\oldpage{221-09}These $s_i$ generate $\OO_{\PP_K^r}(d)$ at the points of $U$, and are thus not all zero, which implies that $d\geq0$.
An easy calculation also shows that
$$
  b_\sh{F} = a_\sh{F}(r+1)/2 + d.
$$
This shows, in particular, that $b_\sh{F}\geq0$, and so $b_\sh{F}$ is bounded below.
...
Suppose, under the preliminary conditions of [(2.1)](#fga-3-iv-theorem-2.1), that, for all $\sh{F}$, we have
$$
  P_\sh{F}(n) = a_\sh{F} n^r/r! + b_\sh{F} n^{r-1}/(r-1)! + \text{terms of degree }<r-1
$$
...
Then the degrees of the $\supp\sh{F}_{(r)}$ are bounded above by $a$, and so, by [(2.4)](#fga-3-iv-lemma-2.4), the $\supp\sh{F}_{(r)}$ form a bounded set.
Furthermore, for each component of $\supp\sh{F}_{(r)}$, the length of $\sh{F}_{(r)}$ for this component is $\leq a$, and so, if $\sh{I}_\sh{F}$ is the ideal that defines $\supp\sh{F}_{(r)}$, then $\sh{F}_{(r)}$ can be considered as a module on the subscheme $Y_\sh{F}$ of $X$ defined by $\sh{I}_\sh{F}^a$.
\oldpage{221-10}As in the previous lemma, we can also reduce to the case where $\sh{F}=\sh{F}_{(r)}$, so that $\sh{F}$ comes from a module on $Y_\sh{F}$.
...
We can then apply [(2.5)](#fga-3-iv-lemma-2.5) to $Y/T$ and $\sh{L}\otimes_X Y$, whence the conclusion.
:::

::: {.proof}
We can now prove [(2.2)](#fga-3-iv-theorem-2.2) by induction on the upper bound $r$ of the $\dim\supp\sh{F}$.
The statement is trivial for $r<0$, so suppose that $r\geq0$ and that the statement has been proven for $r'<r$.
By [(2.6)](#fga-3-iv-lemma-2.6), the $\sh{F}_{(r)}$ form a bounded family, and so too, by part (i) of [(1.2)], do the kernels of the homomorphisms $\sh{L}_K\to\sh{F}_{(r)}$;
...
$$
  P_\sh{F} = P_{\sh{F}_{(r)}} + P_{N_r}
$$
then shows that the $P_{N_r}$ satisfy the same condition (b<sub>s</sub>) as the $P_\sh{F}$.
Thus the $N_r$ satisfy conditions (a) and (b<sub>s</sub>), and so, by the induction hypothesis, the $(N_r)_{(s)}$ are bounded.
But $\sh{F}_{(s)}$ is an extension of $\sh{F}_{(r)}$ by $(N_r)_{(s)}$, and so, by part (ii) of [(1.2)](#fga-3-iv-proposition-1.2), the $\sh{F}_{(s)}$ are bounded.
For the last claim of [(2.2)](#fga-3-iv-theorem-2.2), we note that the kernels $N_s$ of $\sh{F}\to\sh{F}_{(s)}$ are bounded, by part (i) of [(1.2)](#fga-3-iv-proposition-1.2), and that the coefficient of the $n^{s-1}$ term in $P_{N_s}$ is bounded;
...
In what follows, we suppose that $X$ is *projective* over a *Noetherian* $S$, with $\sh{F}$ *coherent*;
...
\oldpage{221-12}With the above notation, the functor $\shQuot_\FXS^P$ is representable by a projective $S$-prescheme $\Quot_\FXS^P$.
:::
...
a. $\RR^i f'_*(\sh{G}(n))=0$ for $i>0$ and $n\geq\nu$;
b. $\RR^i f'_*(\sh{H}(n))=0$ for $i>0$ and $n\geq\nu$;
...
To prove [Theorem 3.2](#fga-3-iv-theorem-3.2), we can easily reduce to the case where $X=\PP_S^r$ (since $S$ is a union of open subsets $U$ such that $X|U$ is a closed subscheme of $\PP_U^r$, and $\OO_X(1)$ is induced by $\OO_{\PP_U^r}(1)$), and where $\sh{F}$ is of the form $(\OO_{\PP_S^r})^N$, and thus flat over $S$.
...
for (a) and (b), this follows in fact from the "Theorem of holomorphic functions" [@GD1960, III, §7], and (c) follows from the Nakayama lemma and the fact that we know that $f'_*(\sh{H}(n+k))=S'_kf'_*(\sh{H}(n))$ anyway for $n$ large enough and $k\geq0$ [@GD1960, III,§2].
\oldpage{221-13}From these remarks, we conclude the following (compare with [@Gro1960a, IV]).
...
ii. Both the subsheaf $\sh{H}$ of $\sh{F}$ defined by the graded submodule $R_\bullet=\sum_{k\geq0}\sh{S}_kR_\nu$ of $M_\bullet$ (cf. [@GD1960, II, §3]) and the quotient $\sh{G}=\sh{F}/\sh{H}$ satisfy conditions (a) and (b) above.

These conditions are clearly necessary, and if they are satisfied then the sheaf $\sh{G}$ defined in (ii) above, being isomorphic to the sheaf associated to the graded $\sh{S}_\bullet$-module $N_\bullet$ given by the sum of the $M_{n+k}/\sh{S}_kR$ is *flat* over $S$ (since its fibres are direct factors of localisations of $N$ for homogeneous prime ideals of $\mathcal{S}_\bullet$), and correspond to the Hilbert polynomial $P$ by virtue of (i).
Taking (ii) into account, we then see that (a') and (a'') are satisfied, and thus, for $n\geq\nu$, the natural homomorphism $N_n\to f_*(\sh{G}(n))$ is a *surjective* homomorphism of locally free modules of equal rank, and thus an isomorphism, and thus $f_*(\sh{H}(\nu+k))=\mathcal{S}_kR_\nu$ for all $k\geq0$, which proves that $\sh{G}\in A_\nu(S)$ and that $R_\nu$ is the element of $\Grass_{P(\nu)}(M_\nu)$ defined by $\sh{G}$.
...
::: {.eqenv #fga-3-iv-equation-asterisk title="" latex=""}
$$
...
To go further, we need to invoke [Theorem 2.1](#fga-3-iv-theorem-2.1), whence we easily conclude that $Q$ is _quasi-compact_ (since it is the image of a prescheme $S'$ of finite type over $S$ that parametrises the family of quotient sheaves of the $\sh{F}_K$ whose Hilbert polynomial is $P$).
...
::: {.proof}
\oldpage{221-17}Indeed, if $\sh{G}_s=\sh{F}_s/\sh{H}_s$, it suffices to consider the largest subsheaf $\sh{H}$ of $\sh{F}$ that induces $\sh{H}_s$ ([@GD1960, I, 9.4.2]) and to take $\sh{G}=\sh{F}/\sh{H}$.
...
Under the conditions of [(3.2)](#fga-3-iv-theorem-3.2), let $Q=\repQuot_{\FXS}^P$, $X_Q=X\times_s Q$, $\sh{F}_Q=\sh{F}\otimes_{\OO_S}\OO_Q$, and let $\sh{G}$ be the coherent quotient of $\sh{F}_Q$, which is flat over $Q$, that has $P$ as its relative Hilbert polynomial, so that $(Q,\sh{G})$ represents the functor $\shQuot_{\FXS}^P$.
...
::: {.proof}
Indeed, we can reduce, as in [(3.2)](#fga-3-iv-theorem-3.2), to the case where $\sh{F}$ is flat over $S$, and then it suffices to take an integer $\nu$ such that $A_\nu=A$ (with the notation above).
:::

The most important application of [(3.2)](#fga-3-iv-theorem-3.2) is in the case where $\sh{F}=\OO_X$.
We then write
$$
  \begin{aligned}
    \repQuot_{\OO_X/X/S}
...
\oldpage{221-18}we note that, when $X=\PP_S^r$, with $S$ the spectrum of a field, the knowledge of the Hilbert polynomial of a coherent module $\sh{F}$ over $X$ is equivalent to that of the Chern classes of $\sh{F}$, or even of the class of $\sh{F}$ in the ring of classes of coherent sheaves on $X$.
...
a. Under the conditions of [(3.1)](#fga-3-iv-theorem-3.1), let $U$ be open in $X$, and denote by $A'$ the subfunctor of $A=\shQuot_{\FXS}$ such that $A'(S')$ is the set of quotient modules $\sh{G}$ of $\sh{F}'$ that are flat over $S'$ and whose support is contained inside $U'$.
  We immediately see that $A'$ is representable by an open subset of the prescheme $\repQuot_{\FXS}$ that represents $A$.
  \oldpage{221-19}It follows that Theorems [(3.1)](#fga-3-iv-theorem-3.1) and [(3.2)](#fga-3-iv-theorem-3.2) remain true if we suppose that $X$ is _quasi-projective_ over $S$ instead of projective over $S$, as long as we also replace in the conclusions the word "projective" by "quasi-projective", and use $\shQuot_{\FXS}(S')$ to mean the set of coherent quotients $\sh{G}$ of $\sh{F}'$ that are flat over $S'$ and _whose support is proper over $S'$._
...
c. The preschemes $\repHom_S(X,Y)$, $\prod_{X/S}Z/S$, and $\repIsom_S(X,Y)$, defined in [FGA 3.II, C.2](#fga-3-ii-section-C.2) exist thanks to suitable projective hypotheses, and can be realised as opens in suitable Hilbert preschemes.
  ...
  \oldpage{221-20}In this way, _if $X$ is flat and proper over $S$, and $Z$ quasi-projective over $S$, then $\prod_{X/S}(Z/X)$ exists and is realised as an open subprescheme of $\repHilb_{Z/S}$._
...
Suppose that $Q=\repQuot_\FXS$ exists (cf. [§4.a](#fga-3-iv-section-4)) --- for example, suppose that $X$ is quasi-projective over some locally Noetherian $S$, and $\sh{F}$ is coherent.
Let $x\in Q$, corresponding to a residue extension $K=k(x)$ of some $k(s)$ (where $s\in S$).
\oldpage{221-22}Then $x$ is defined by a coherent quotient module $\sh{G}_0=\sh{F}_0/\sh{H}_0$ of the module $\sh{F}_0=F_K$ on the $K$-prescheme $X_K$.
...
It suffices to apply [(5.1)](#fga-3-iv-proposition-5.1) by replacing $S_0$ with $S'$, and $S$ with the prescheme $D(\sh{M})=(S',\OO_{S'}+\sh{M})$, where $\sh{M}$ is considered as a square-zero ideal.

If, in [(5.1)](#fga-3-iv-proposition-5.1), we have $\sh{F}=\OO_X$, then the data of $\sh{G}_0$ corresponds to the data of a closed subprescheme $Y_0$ of $X_0$ that is flat over $S_0$, defined by the ideal $\sh{J}_0=\sh{M}_0$, and then [($*$)](#fga-3-iv-equation-asterisk) gives
$$
  \sh{A} = \repHom_{\OO_{X_0}}(\sh{J}_0/\sh{J}_0^2,\OO_{Y_0}\otimes_{\OO_{S_0}}\sh{J})
$$
where $\sh{J}/\sh{J}^2$ is thought of as the *conormal sheaf* of $Y_0$ in $X_0$, which we also denote by $\sh{N}_{Y_0/X_0}$;
\oldpage{221-23}it is then interesting to consider $\sh{A}$ as a module over $Y_0$, and to calculate $\HH^0$ and $\HH^1$ on $Y$.
If $Y_0$ is locally a complete intersection in $X_0$, with $X$ flat over $S$, then, in [(5.1)](#fga-3-iv-proposition-5.1), the possibility of a local extension is guaranteed, and $\sh{J}/\sh{J}^2$ is locally free over $Y_0$ and we can write
$$
...
Under the conditions of [(5.3)](#fga-3-iv-corollary-5.3), suppose that $\sh{F}=\OO_X$, with $X$ flat over $S$, and that the closed subprescheme $Y_0$ of $X_0$ that corresponds to $\sh{G}_0$ is locally a complete intersection.
...
\oldpage{221-24}In fact, Zappa [@Zap1945] constructed an example (with $X$ a non-singular projective surface over the *field of complex numbers*) where even at the generic point of $T$ the characteristic homomorphism is not surjective.
...
Kodaira gave a different criterion in [@Kod1956], given by the vanishing of $\HH^1(X_0,\sh{L})$, where $\sh{L}=\sh{J}_0^{-1}$ is the invertible sheaf on $X_0$ defined by the divisor $Y_0$;
...
$$
\oldpage{221-26}which allows us to recover the $\sh{N}_{X/S}^u(\sh{F})$.
...
we will not give here the details of the proof, which also uses the result of factorisation of a finite morphism given in [FGA 3.I, A.2.b](#fga-3-i-section-A.2.b).
