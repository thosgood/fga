## Bounded sets of sheaves: invariance properties {#fga3.iv-1}
...
\oldpage{221-02}
So, if $K$ and $K'$ are arbitrary extensions of $k$, and $\sh{F}$ a quasi-coherent sheaf on $X_K$ and $\sh{F}'$ a quasi-coherent sheaf on $X_{K'}$, then we say that $\sh{F}$ and $\sh{F}'$ are _equivalent_ if there exists an extension $K''/k$ as well as $k$-homomorphisms $K\to K''$ and $K'\to K''$ such that $\sh{F}_{K''}$ and $\sh{F}'_{K''}$ are isomorphic over $X_{K''}$.
...
::: {.rmenv #fga3.iv-definition-1.1 title="Definition 1.1" latex="{Definition 1.1}"}
\oldpage{221-03}
...
::: {.itenv #fga3.iv-proposition-1.2 title="Proposition 1.2" latex="{Proposition 1.2}"}
...
::: {.itenv #fga3.iv-proposition-1.3 title="Proposition 1.3" latex="{Proposition 1.3}"}
...
We can prove the analogous result to [(1.3)](#fga3.iv-proposition-1.3) for the sheaves canonically induced from $\sh{F}$ by the theory of primary decomposition;
...
## Bounded families and the Hilbert polynomial {#fga3.iv-2}
...
::: {.itenv #fga3.iv-theorem-2.1 title="Theorem 2.1" latex="{Theorem 2.1}"}
...
::: {.itenv #fga3.iv-theorem-2.2 title="Theorem 2.2" latex="{Theorem 2.2}"}
Under the conditions of [Theorem 2.1](#fga3.iv-theorem-2.1), let $s$ be an integer, and suppose that $E$ satisfies condition (a), as well as the following weakened form of (b):
...
::: {.itenv #fga3.iv-corollary-2.3 title="Corollary 2.3" latex="{Corollary 2.3}"}
...
Then, in condition (b) of [Theorem 2.1](#fga3.iv-theorem-2.1), we can restrict to the coefficients of $P_\sh{F}$ between degree $(s-1)$ and $r$, inclusive.
:::

The end of this section is dedicated to a sketch proof of [Theorem 2.2](#fga3.iv-theorem-2.2).
...
::: {.itenv #fga3.iv-lemma-2.4 title="Lemma 2.4 (Chow)" latex="{Lemma 2.4 (Chow)}"}
...
::: {.itenv #fga3.iv-lemma-2.5 title="Lemma 2.5" latex="{Lemma 2.5}"}
...
We then conclude by part (i) of [(1.2)](#fga3.iv-proposition-1.2).
:::

The combination of the two lemmas above allows us to prove the following:

::: {.itenv #fga3.iv-lemma-2.6 title="Lemma 2.6" latex="{Lemma 2.6}"}
Suppose, under the preliminary conditions of [(2.1)](#fga3.iv-theorem-2.1), that, for all $\sh{F}$, we have
$$
  P_\sh{F}(n) = a_\sh{F} n^r/r! + b_\sh{F} n^{r-1}/(r-1)! + \text{terms of degree }<r-1
$$
...
Then the degrees of the $\supp\sh{F}_{(r)}$ are bounded above by $a$, and so, by [(2.4)](#fga3.iv-lemma-2.4), the $\supp\sh{F}_{(r)}$ form a bounded set.
...
We can then apply [(2.5)](#fga3.iv-lemma-2.5) to $Y/T$ and $\sh{L}\otimes_X Y$, whence the conclusion.
:::

::: {.proof}
We can now prove [(2.2)](#fga3.iv-theorem-2.2) by induction on the upper bound $r$ of the $\dim\supp\sh{F}$.
The statement is trivial for $r<0$, so suppose that $r\geqslant0$ and that the statement has been proven for $r'<r$.
By [(2.6)](#fga3.iv-lemma-2.6), the $\sh{F}_{(r)}$ form a bounded family, and so too, by part (i) of [(1.2)], do the kernels of the homomorphisms $\sh{L}_K\to\sh{F}_{(r)}$;
...
But $\sh{F}_{(s)}$ is an extension of $\sh{F}_{(r)}$ by $(N_r)_{(s)}$, and so, by part (ii) of [(1.2)](#fga3.iv-proposition-1.2), the $\sh{F}_{(s)}$ are bounded.
For the last claim of [(2.2)](#fga3.iv-theorem-2.2), we note that the kernels $N_s$ of $\sh{F}\to\sh{F}_{(s)}$ are bounded, by part (i) of [(1.2)](#fga3.iv-proposition-1.2), and that the coefficient of the $n^{s-1}$ term in $P_{N_s}$ is bounded;
then [(2.6)](#fga3.iv-lemma-2.6) proves that the coefficient of the following term is bounded below.
...
::: {.proof}
Returning to the proof of [(3.4)](#fga3.iv-lemma-3.4) where we left off, we denote by $Z_n$ the _closed_ subprescheme of $S$ associated, by [(3.6)](#fga3.iv-lemma-3.6), to the module $M_n$ and the integer $r=P(n)$, and by $Z'_N$ the infimum of the $Z_n$ for $\nu\leqslant n\leqslant N$.
...
To go further, we need to invoke [Theorem 2.1](#fga3.iv-theorem-2.1), whence we easily conclude that $Q$ is _quasi-compact_ (since it is the image of a prescheme $S'$ of finite type over $S$ that parametrises the family of quotient sheaves of the $\sh{F}_K$ whose Hilbert polynomial is $P$).
Thus $Q$ is equal to one of the $Q_\nu$, and thus quasi-projective over $S$.
To prove that it is projective over $S$, it thus remains to prove that it is _proper_ over $S$, and for this it suffices to invoke the valuative criterion of properness in the form given in [@GD1960, II, 7.3.8].
...
::: {.proof}
\oldpage{221-17}
Indeed, if $\sh{G}_s=\sh{F}_s/\sh{H}_s$, it suffices to consider the largest subsheaf $\sh{H}$ of $\sh{F}$ that induces $\sh{H}_s$ ([@GD1960, I, 9.4.2]) and to take $\sh{G}=\sh{F}/\sh{H}$.
...
a. Under the conditions of [(3.1)](#fga3.iv-theorem-3.1), let $U$ be open in $X$, and denote by $A'$ the subfunctor of $A=\shQuot_{\FXS}$ such that $A'(S')$ is the set of quotient modules $\sh{G}$ of $\sh{F}'$ that are flat over $S'$ and whose support is contained inside $U'$.
  ...
  \oldpage{221-19}
  It follows that Theorems [(3.1)](#fga3.iv-theorem-3.1) and [(3.2)](#fga3.iv-theorem-3.2) remain true if we suppose that $X$ is _quasi-projective_ over $S$ instead of projective over $S$, as long as we also replace in the conclusions the word "projective" by "quasi-projective", and use $\shQuot_{\FXS}(S')$ to mean the set of coherent quotients $\sh{G}$ of $\sh{F}'$ that are flat over $S'$ and _whose support is proper over $S'$._
...
::: {.itenv #fga3.iv-corollary-5.2 title="Corollary 5.2"}
...
::: {.itenv #fga3.iv-corollary-5.3 title="Corollary 5.3"}
...
::: {.itenv #fga3.iv-corollary-5.4 title="Corollary 5.4"}
...
::: {.rmenv #fga3.iv-remark-5.5 title="Remark 5.5"}
...
::: {.rmenv #fga3.iv-remark-5.6 title="Remark 5.6"}
We have given, in [(5.4)](#fga3.iv-remark-5.4), a criterion for simplicity, which applies in particular to schemes of divisors.
...
::: {.itenv #fga3.iv-proposition-5.7 title="Proposition 5.7"}
