...
$$
\oldpage{236-15}
by the argument of Serre, if the $X_s$ are separable (without even supposing the existence of $\repPic_{X/S}^\tau$), or if the ${}_n\repPic_{X/S}$ (kernels of the $\varphi_n$ into the Picard functor) are *separated* over $S$ (taking into account the fact that they are étale over $S$, thanks to [(2.5)](#fga3.vi-theorem-2.5)).
...
We recall, however, that, if the geometric fibres of $\repPic_{X/S}$ are *reduced* and have no additive component, then it follows from [(1.8)](#fga3.vi-corollary-1.8) and [(2.5)](#fga3.vi-theorem-2.5) that $\repPic_{X/S}$ is simple over $S$ at the points of $\repPic_{X/S}^\sigma$ whenever $S$ is reduced;
...
\oldpage{236-22}
It is possible, however, that this is the case if $f$ is *separable* and has *irreducible geometric fibres*.
...
b'. (If the fibres of $X/S$ are all of the same dimension $n$).^[*[Trans.]* Conditions (c) and (d) were originally called (b') and (b'').]
...
\oldpage{C-
11}We note finally that the proof of (i) and (ii) also shows that, in the case where $S$ is the spectrum of a field $k$, the morphism $\repPic_{Y/k}\to\repPic_{X/k}$ is *affine* (and not only of finite type).
:::
