...
Note that $X\mapsto\Pic(X)$ is a contravariant functor in $X$ in the evident way, and that the isomorphism \ref{fga3.v-equation-1.1} is functorial.
...
$$
  ...
  = \Pic(X\times_S T)
$$
the contravariant functor on $\Sch_{/S}$ defined by [(1.6)](#fga3.v-equation-1.6) is called the \em{relative Picard functor} of $X$ over $S$, and denoted by $\shPic_{X/S}$, and we define the \em{relative Picard group} of $X$ over $S$, denoted by $\Pic(X/S)$, the group $\shPic_{X/S}(S)$.
...
By [(2.3)](#fga3.v-corollary-2.3) below, if we replace $P$ by $P'$ in the description of $\Pic(X/S)$ that we have just given then we can take $S''=S'\times_S S'$ and $S'_1=S'\times_S S_1$, at least under the conditions given in \em{loc. cit.}.
...
\oldpage{232-13}
Condition (a) of \em{loc. cit.}, namely that $(+)$ is representable by morphisms that are elements of $\mathcal{S}$, is satisfied as we have just seen;
